...
def train_epoch(data_loader, model, optimizer):
    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            ...
            level_k_outputs, ego_plan, probability = model(inputs)
            ade = torch.norm(ego_plan[..., :2] - ego_future[:, None, :, :2], dim=-1)
            best_mode = torch.argmin(ade.sum(-1), dim=-1)
            best_traj = ego_plan[torch.arange(ego_plan.shape[0]), best_mode]
            ego_plan = best_traj

...
def create_map_raster(lanes, crosswalks, route_lanes):
    ...
    for k in range(route_lanes.shape[0]):
        ...
        plt.scatter(route_lane[0, 0], route_lane[0, 1], s=30, c='r')
        plt.scatter(route_lane[-1, 0], route_lane[-1, 1], s=30, c='k')
        if route_lane[0][0] != 0:
            # print(k)
            plt.plot(route_lane[1:-1, 0], route_lane[1:-1, 1], 'g', linewidth=1) # plot route_lanes

...
class Encoder(nn.Module):
    def __init__(self, dim=256, layers=6, heads=8, dropout=0.1,
        ...
        self.attn = Attention(dim, num_heads=heads, qkv_bias=True, attn_drop=0., proj_drop=0.)
        self.block = Block(dim=dim, num_heads=heads)
        depth = 4
        ...
        norm_layer = norm_layer or partial(nn.LayerNorm, eps=1e-6)# stochastic depth decay rule
        ...
        self.pruning_loc = [1, 2, 3]  # [3, 6, 9]
        token_ratio = [0.7, 0.49, 0.343]
    ...
    def forward(self, inputs, keep_rate):
        ...
        token_idx = torch.arange(0, num_token_all - 1).long().unsqueeze(0).expand(B, -1).to(input.device)
        token_idx_agent = torch.arange(0, 20).long().unsqueeze(0).expand(B, -1).to(input.device)
        token_idx_map = torch.arange(0, num_token_all - 21).long().unsqueeze(0).expand(B, -1).to(input.device)# (B, N)      initial
        # attn_mask = mask.clone()
        attn_mask = torch.ones(B, N1, N1).to(input.device) # (B, N+1, N+1) 1

        # input, token_idx_agent, token_idx_map, attn_mask = self.block(input, token_keep_ratio= 0.9, token_idx_agent=token_idx_agent, token_idx_map=token_idx_map, token_prune=True, attn_prune=True, attn_mask=attn_mask, attn_keep_ratio=0.9)

        # x_att, cls_attn, patch_attn = self.attn(self.norm1(input), mask)


        # encoding = self.fusion_encoder(input, src_key_padding_mask=mask)

        # input = encoding
        ratio = attn_ratio = keep_rate
        # Transformer Block >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        for i, blk in enumerate(self.blocks):
            if i in self.pruning_loc:           # i = 3, 6, 9
                input, token_idx_agent, token_idx_map, attn_mask = blk(input, token_keep_ratio=ratio, token_idx_agent=token_idx_agent, token_idx_map=token_idx_map, token_prune=True, attn_prune=True, attn_mask=attn_mask, key_padding_mask=mask, attn_keep_ratio=attn_ratio)
                ego_mask = mask[:, 0:1]
                # actors_mask = batched_index_select(mask[:, 1:token_idx_agent.shape[1]+1], 1, token_idx_agent)
                ...
                input, token_idx_agent, token_idx_map, attn_mask = blk(input, token_keep_ratio=ratio, token_idx_agent=token_idx_agent, token_idx_map=token_idx_map, token_prune=False, attn_prune=False,  attn_mask=attn_mask, key_padding_mask=mask, attn_keep_ratio=attn_ratio)
                # ego_mask = mask[:, 0:1]
                # actors_mask = batched_index_select(mask[:, 1:21], 1, token_idx_agent)
                # map_mask = batched_index_select(mask[:, 21:], 1, token_idx_map)
                # mask = torch.cat([ego_mask, actors_mask, map_mask], dim=1)
                # actors = batched_index_select(actors, 1, token_idx_agent)
                # attn_mask = mask
        #
...
class Decoder(nn.Module):
    ...
    def forward(self, encoder_outputs):
        ...
        for k in range(1, self.levels + 1):
            ...
            last_content, last_level, last_score = interaction_decoder(current_states, last_level, last_score,
                                                                       last_content, encoding, mask)
...
class NeuralPlanner(nn.Module):
    ...
    def forward(self, env_encoding, route_lanes, initial_state):
        ...
        predictions, probability = self.plan_predicter(env_route_encoding)
...
class GameFormer(nn.Module):
    ...
    def forward(self, inputs, keep_rate):
        encoder_outputs = self.encoder(inputs, keep_rate)
        route_lanes = encoder_outputs['route_lanes']

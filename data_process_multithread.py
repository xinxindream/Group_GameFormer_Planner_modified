...
class DataProcessor(object):
    def __init__(self, scenarios):
        ...
        self._max_elements = {'LANE': 40, 'ROUTE_LANES': 10,
                              'CROSSWALK': 5}  # maximum number of elements to extract per feature layer.
        self._max_points = {'LANE': 50, 'ROUTE_LANES': 50,
                            'CROSSWALK': 30}  # maximum number of points per feature to extract per feature layer.
    ...
    def work(self, save_dir, debug=False):
        for scenario in tqdm(self._scenarios):
            ...
            ego_agent_past, neighbor_agents_past, neighbor_indices = \
                agent_past_process(ego_agent_past, time_stamps_past, neighbor_agents_past, neighbor_agents_types,
                                   self.num_agents)
    ...
    def work_multithread(self, save_dir, debug=False):
        import concurrent.futures
        with concurrent.futures.ThreadPoolExecutor(max_workers = 16) as executor:
            results = executor.map(self.work_one, self._scenarios)
            for result in tqdm(results):
                self.save_to_disk(save_dir, result)

    def work_one(self, scenario):
        map_name = scenario._map_name
        token = scenario.token
        self.scenario = scenario
        self.map_api = scenario.map_api
...
if __name__ == "__main__":
    ...
    parser.add_argument('--scenarios_per_type', type=int, default=20, help='number of scenarios per type')
    parser.add_argument('--total_scenarios', default=None, help='limit total number of scenarios')
    parser.add_argument('--val_ratio', type=float, default=0.1, help='ratio of val_scenarios')
    ...
    val_num = int(len(scenarios) * args.val_ratio)
    selected_val_scenarios = random.sample(scenarios, val_num)
    selected_train_scenarios = list(set(scenarios) - set(selected_val_scenarios))

    # process data
    del worker, builder, scenario_filter, scenario_mapping
    # processor = DataProcessor(selected_train_scenarios)
    # processor.work(args.save_path, debug=args.debug)
    processor_val = DataProcessor(selected_train_scenarios)
    import time
    start_time = time.time()  # 记录算法开始时间
    processor_val.work_multithread(args.save_path + '_val', debug=args.debug)
    end_time = time.time()  # 记录算法结束时间
    elapsed_time = end_time - start_time  # 计算算法运行时间
    print(f'My algorithm takes {elapsed_time:.2f} seconds to run.')
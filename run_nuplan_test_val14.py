...
def main(args):
    ...
    scenario_filter = ScenarioFilter(*get_filter_parameters(args.scenarios_per_type, args.total_scenarios, args.shuffle_scenarios, args.scenario_tokens, args.timestamp_threshold_s))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--total_scenarios', default=2, help='limit total number of scenarios')
    parser.add_argument('--shuffle_scenarios', type=bool, default=False, help='shuffle scenarios')
    parser.add_argument('--config_path', type=str, default='/home/xingchen24/code/deeplearning/imitation_learning/PlanTF/planTF/config/scenario_filter/val14.yaml', help='`config path')

    args = parser.parse_args()

    with open(args.config_path, "r") as file:
        conf = yaml.safe_load(file)
    args.scenarios_per_type = conf['num_scenarios_per_type']
    args.total_scenarios = conf['limit_total_scenarios']
    args.shuffle_scenarios = conf['shuffle']
    args.scenario_tokens = conf['scenario_tokens']
    args.timestamp_threshold_s = conf['timestamp_threshold_s']
    main(args)

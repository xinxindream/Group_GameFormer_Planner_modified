...
def train_epoch(data_loader, model, optimizer, epoch):
    ...
    with tqdm(data_loader, desc="Training", unit="batch") as data_epoch:
        for batch in data_epoch:
            ...
            level_k_outputs, ego_multi_plan, probability = model(inputs,keep_rate)
            ...
            plan_loss = planning_loss(ego_plan, ego_future, ego_multi_plan, level_k_outputs[f'level_{len(level_k_outputs) // 2 - 1}_interactions'], neighbors_future_valid)

            # ego-plan的得分的损失 来自于plantf
            ego_cls_loss = F.cross_entropy(probability, best_mode.detach())
            plan_loss += ego_cls_loss
        ...
        for batch in data_epoch:
           ...
           # prepare data
            ...
            with torch.no_grad():
                ...
                loss, results = level_k_loss(level_k_outputs, ego_future, neighbors_future, neighbors_future_valid)
                prediction = results[:, 1:]
                plan_loss = planning_loss(ego_plan, ego_future, ego_multi_plan,
                                          level_k_outputs[f'level_{len(level_k_outputs) // 2 - 1}_interactions'],
